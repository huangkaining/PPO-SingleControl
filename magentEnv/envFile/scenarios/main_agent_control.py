...
from magentEnv.envFile.scenario import BaseScenario
...
class Scenario(BaseScenario):
    def make_world(self):
        ...
        #add controller
        world.controllers = [Controller() for i in range(num_controllers)]
        for i,co in enumerate(world.controllers):
            co.collide =False
    ...
    def reset_world(self, world):
        ...
        for agent in world.agents:
            ...
            if agent.adversary:
                agent.state.p_pos = np.random.uniform(-1, -0.8, world.dim_p)
            else:
                agent.state.p_pos = np.random.uniform(0.6, 1, world.dim_p)
